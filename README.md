...
Raw packet access needs elevated privileges. Because `sudo` resets `PATH` to the system Python, running `sudo python scanner.py` from inside an activated venv will fail with `ModuleNotFoundError: No module named 'scapy'` even though it's installed — sudo isn't seeing your venv. Use one of:
